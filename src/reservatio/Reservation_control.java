...
//////新規予約の登録
	...
	public String yoyakukakunin(Reservation_view frame){
		String res ="";
		...
		if(flagLogin){
			try {
				...
				ResultSet rs = mysql.yoyakukakunin(reservation_userid);
				boolean exist = false;
				while (rs.next()) {
    ...
    public String gaiyou(Reservation_view frame){
		String res = "";
		MySQL mysql = new MySQL();
		
		 try {
			 ResultSet rs;
			 rs = mysql.gaiyou(frame.choiceFacility.getSelectedItem());
			 while (rs.next()) {
